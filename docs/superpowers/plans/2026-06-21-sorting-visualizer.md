...
- [x] **Step 5: 전체 알고리즘 테스트 한 번에 실행해서 회귀 확인**
...
- [x] **Step 3: `SortVisualizer` 클래스(메뉴 화면) 구현**
...
- [x] **Step 2: 디스플레이가 있는 환경에서 수동 실행 검증**

이 작업 환경(WSL2, Xvfb)에는 실제 사람이 마우스로 클릭할 수 있는
디스플레이가 없어, 동일한 코드 경로를 호출하는 스크립트로 각 화면을
`savefig`로 이미지화한 뒤 직접 시각적으로 검토하는 방식으로 대체
검증했다 (메뉴 → 퀵 정렬 중간 프레임 → 버블 정렬 완료 → 비교 설정 →
실제 `run_comparison` 결과 차트, 총 5개 화면).

확인 결과:
- [x] 메뉴 화면에 6개 알고리즘 버튼과 "전체 성능 비교" 버튼이 보인다
- [x] 정렬 중간 프레임에서 비교 중(빨강)/교환 중(초록) 색상이 정확한
      인덱스에 적용된다
- [x] 정렬 완료 프레임에서 모든 막대가 진한 파랑(정렬 완료)이고
      오름차순으로 배열되어 있다
- [x] 상단 텍스트에 비교 횟수/배열 접근/경과 시간이 표시된다
      (예: "비교 횟수: 184  배열 접근: 250  경과 시간: 0.23s")
- [x] 배열 크기·속도 슬라이더와 재시작/일시정지/메뉴로 버튼이 화면에
      모두 렌더링된다 (`_restart_sort`/`_toggle_pause`는 Task 12의
      자동화 테스트에서 직접 호출까지 검증됨)
- [x] "전체 성능 비교" 결과가 비교 횟수/실행 시간(ms) 막대그래프
      두 장으로 표시되고, 값 라벨도 정확히 찍힌다 (n=40에서
      버블/선택/삽입은 163~780, 병합/퀵/힙은 163~310으로 O(n²) vs
      O(n log n) 격차가 수치로 드러남)
- [ ] 한글 텍스트(버튼 라벨, 제목, 축 이름)가 깨지지 않고 표시된다
      — **이 작업 환경(Linux, 한글 폰트 미설치)에서는 확인 불가.**
      `fc-list`로 확인한 결과 나눔고딕 등 한글 폰트가 시스템에 없어
      모든 한글 글리프가 빈 사각형(tofu)으로 렌더링되었다. 이는
      `font_utils.detect_korean_font()`가 정확히 대비하는 상황이며
      (Task 8에서 OS별 폰트 매핑과 "못 찾으면 경고만 출력" 동작을
      단위 테스트로 이미 검증함), Windows/Mac처럼 한글 폰트가 기본
      설치된 환경에서는 정상 렌더링될 것으로 예상된다. **사용자가
      자신의 Windows/Mac 환경에서 `python main.py`로 최종 확인 필요.**
